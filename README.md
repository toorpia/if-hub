...
#### processors/
データ処理用の外部プロセッサモジュールが格納されています。これらのモジュールは主にPythonで実装されており、移動平均やZスコア計算などの時系列データ処理を担当します。

#### gtags/
仮想タグ（Generated Tags）の定義と計算ロジックが格納されています。定義ファイル（JSON形式）には計算式や参照するタグ情報が含まれ、必要に応じてPythonスクリプトによる高度な計算も可能です。
