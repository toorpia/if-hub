...
- **ダッシュボード** (`frontend/`) - システム監視用Webダッシュボード
...
> 詳細は「[本番環境デプロイガイド](docs/ja/deployment_guide.md)」を必ずご確認ください。
...
- **`frontend/`**: ダッシュボードのソースコード（Vue.js + Vite）
- **`public/`**: ビルド済みダッシュボード（本番環境で使用）
...
Docker環境では、`docker-compose.timescaledb.yml`で設定されたポート（デフォルト: 3001）でアクセスします。
...
開発サーバーは `http://localhost:5173` で起動し、APIリクエストは自動的に `localhost:3001` にプロキシされます。
