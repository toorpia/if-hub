...
required_files=("if-hub-container.tar" "pi-ingester-container.tar" "docker-compose.yml")

for file in "${required_files[@]}"; do
    if [ ! -f "$file" ]; then
        echo "❌ 必要なファイルが見つかりません: $file"
        echo "   パッケージが正しく展開されているか確認してください"
        exit 1
    fi
    echo "  ✅ $file"
done
...
if docker compose down 2>/dev/null; then
    ...
    echo "   ℹ️  停止すべきコンテナがありませんでした"
fi
echo ""

# コンテナの起動（インポートしたイメージを使用して強制再作成）
echo "🚀 コンテナを起動しています..."
